...
import com.example.hotelbackend.booking.mapper.BookingWithRoomDtoMapper;
...
public class BookingService {
    ...
    private final BookingWithRoomDtoMapper bookingWithRoomDtoMapper;
    private final ClientSMTP clientSMTP;

    public BookingService(BookingRepository bookingRepository, BookingDtoMapper bookingDtoMapper, BookingWithRoomDtoMapper bookingWithRoomDtoMapper, ClientSMTP clientSMTP) {
        this.bookingRepository = bookingRepository;
        this.bookingDtoMapper = bookingDtoMapper;
        this.bookingWithRoomDtoMapper = bookingWithRoomDtoMapper;
        this.clientSMTP = clientSMTP;
    }

    public Long saveBooking(BookingWithIdsDto bookingWithIdsDto) {
    ...
    public List<BookingWithRoomDto> getAllUserBookings(Long userId) {
        return bookingRepository.findAllByCustomerId(userId).stream().map(bookingWithRoomDtoMapper::map).toList();
    }
    ...
    private String generateEmailConfirmationText(Booking booking) {
        return "Cześć " + booking.getCustomer().getName() + "!\n\n" + "Dziękujemy za dokonanie rezerwacji! Oto szczegóły Twojej rezerwacji:\n\n" + "Numer rezerwacji: " + booking.getId() + "\n" + "Data zameldowania: " + booking.getCheckInDate() + "\n" + "Data wymeldowania: " + booking.getCheckOutDate() + "\n" + "Pokój: " + booking.getRoom().getName() + "\n" + "Cena całkowita: " + booking.getBookingPrice() + " PLN\n\n" + "Z niecierpliwością oczekujemy na Twoje przybycie.\n\n" + "Z poważaniem,\n" + "Zespół Royal Residence";
    }
